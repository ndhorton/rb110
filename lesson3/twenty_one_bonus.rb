...
def card_value(card)
  if FACE_CARDS.include?(card.first) then 10
  elsif card.first == TEXT[:ace]     then 1
  else
    card.first
  end
end
...
def calculate_result(player_hand, dealer_hand)
  if bust?(player_hand)                         then :player_bust
  elsif bust?(dealer_hand)                      then :dealer_bust
  elsif score(player_hand) > score(dealer_hand) then :player_won
  elsif score(dealer_hand) > score(player_hand) then :dealer_won
  else
    :tie
  end
end
...
loop do
  ...
  if bust?(player_hand)
    ...
    play_again.downcase.start_with?('y') ? next : break
  end
